...
(function (t, e) {
	...
	function r(n, r, l) {
		...
		function S() {
			ne = r.contentHeight ? r.contentHeight : r.height ? r.height - (Q ? Q.height() : 0) - L(G) : Math.round(G.width() / Math.max(r.aspectRatio, 0.5));
		}
		function E(t) {
		...
		function s(r, a) {
			...
			if (l) {
				...
				if (u) {
					...
					m && (g[m] = Math.round(+S / 1000)), b && (g[b] = Math.round(+E / 1000)), p(), t.ajax(t.extend({}, Me, r, {data:g, success:function (e) {
						e = e || [];
	...
	function m(t, n) {
		return "object" == typeof t ? t : "number" == typeof t ? new Date(1000 * t) : "string" == typeof t ? t.match(/^\d+(\.\d+)?$/) ? new Date(1000 * parseFloat(t)) : (n === e && (n = !0), y(t, n) || (t ? new Date(t) : null)) : null;
	}
	function y(t, e) {
		...
		if (e || !n[13]) {
			...
			n[3] && (r.setMonth(n[3] - 1), a.setMonth(n[3] - 1)), n[5] && (r.setDate(n[5]), a.setDate(n[5])), l(r, a), n[7] && r.setHours(n[7]), n[8] && r.setMinutes(n[8]), n[10] && r.setSeconds(n[10]), n[12] && r.setMilliseconds(1000 * Number("0." + n[12])), l(r, a);
		} else {
			if (r.setUTCFullYear(n[1], n[3] ? n[3] - 1 : 0, n[5] || 1), r.setUTCHours(n[7] || 0, n[8] || 0, n[10] || 0, n[12] ? 1000 * Number("0." + n[12]) : 0), n[14]) {
				var o = 60 * Number(n[16]) + (n[18] ? Number(n[18]) : 0);
				o *= "-" == n[15] ? 1 : -1, r = new Date(+r + 1000 * 60 * o);
			}
		}
		return r;
	}
	...
	function M(t) {
		...
		return n.setDate(n.getDate() + 4 - (n.getDay() || 7)), e = n.getTime(), n.setMonth(0), n.setDate(1), Math.floor(Math.round((e - n) / 86400000) / 7) + 1;
	}
	function C(t) {
	...
	function ce() {
		...
		function s(n, r) {
			...
			for ((T = y("isRTL")) ? (k = -1, z = Y - 1) : (k = 1, z = 0), a = 0; F > a; a++) {
				o = n[a], i = o.event, s = B(o.start, o.start), c = B(o.start, o.end), u = o.col, f = o.level, d = o.forward || 0, h = q(u * k + z), g = I(u * k + z) - h, g = Math.min(g - 6, 0.95 * g), p = f ? g / (f + d + 1) : d ? 2 * (g / (d + 1) - 6) : g, m = h + g / (f + d + 1) * f * k + (T ? g - p : 0), o.top = s, o.left = m, o.outerWidth = p, o.outerHeight = c - s, N += l(i, o);
			}
	...
	var Se = ["sun", "mon", "tue", "wed", "thu", "fri", "sat"], Ee = 86400000, xe = 3600000, Te = 60000, ke = {s:function (t) {
		return t.getSeconds();
	}, ss:function (t) {
	...
	we.dateFormatters = ke, we.applyAll = G, De.month = te, De.basicWeek = ee, De.basicDay = ne, n({weekMode:"fixed"}), De.agendaWeek = oe, De.agendaDay = ie, n({allDaySlot:!0, allDayText:"all-day", firstHour:6, slotMinutes:30, defaultEventMinutes:120, axisFormat:"h(:mm)tt", timeFormat:{agenda:"h:mm{ - h:mm}"}, dragOpacity:{agenda:0.5}, minTime:0, maxTime:24});
})(jQuery);
